...
def get_context_usage(claude_data):
    ...
    try:
        ...
        if 'model' in claude_data:
            model_id = claude_data['model'].get('id', '')
        ...
        # If we have the exceeds_200k_tokens flag, we can infer usage
        if claude_data.get('exceeds_200k_tokens', False) and context_tokens == 0:
            context_tokens = context_limit  # At or near limit
...
def calculate_status(claude_data=None):
    ...
    if 'workspace' in claude_data:
        cwd = claude_data['workspace'].get('current_dir') or cwd
    ...
    if 'cost' in claude_data:
        session_cost = claude_data['cost'].get('total_cost_usd', 0.0)
        session_found = True
